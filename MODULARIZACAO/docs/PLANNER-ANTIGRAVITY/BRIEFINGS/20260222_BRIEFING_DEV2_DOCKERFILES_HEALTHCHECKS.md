...
> [!CAUTION]
> O Zilda usa `context: .` (raiz do MODULARIZACAO) no `docker-compose.full.yml`, mas o Dockerfile atual usa `COPY pyproject.toml .` como se o context fosse o diretório do módulo. **Precisa mudar para `COPY ./intellicare-zilda/...`** para ser consistente.
...
> [!IMPORTANT]
> O build context no `docker-compose.full.yml` usa `context: .` (raiz do MODULARIZACAO). Então os COPYs nos Dockerfiles devem usar caminhos relativos a essa raiz:
> - ✅ `COPY ./intellicare-core /tmp/intellicare-core`
