# Plano de Implementação — Configuração do Servidor de Homologação

**Fase:** 1 — Preparação do Sistema (implantação do servidor)  
**Data:** 2026-02-21  
**Servidor:** 167.86.97.142 (Contabo VPS)  
**Objetivo:** Configurar servidor para deploy do IntelliCare v1.0.0  
**Destinatário:** DEV (execução)
...
| **D** | Deploy completo (backends + portal) | ⚠️ Após Estabilização concluída |
...
- [ ] Estabilização do projeto concluída (para Fase D)
...
```bash
cd /opt/intellicare/intellicare/MODULARIZACAO  # ou path correto
...
**Executar quando Estabilização estiver concluída.**
...
```
Hoje (sem depender de Estabilização):
  A1 → A2 → A3 → A4 → A5 → B1 → B2 → B3 → C1 → C2 → C3

Quando Estabilização concluída:
