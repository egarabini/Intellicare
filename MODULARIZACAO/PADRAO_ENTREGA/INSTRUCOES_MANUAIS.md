...
scp C:\DOCSHARE\INTELLICARE\MODULARIZACAO\PADRAO_ENTREGA\EXECUCAO_SERVIDOR_COMPLETA.sh root@167.86.97.142:/root/
```
...
# Backup do .env
cp /opt/intellicare/MODULARIZACAO/.env /opt/intellicare/backups/manual_$(date +%Y%m%d_%H%M%S)/.env.backup
```
...
# Atualizar .env
cd /opt/intellicare/MODULARIZACAO
...
ssh root@$SERVER "cd /opt/intellicare/MODULARIZACAO && git fetch --all && git checkout chore/v1-close-staging-standard && git pull --ff-only && docker compose --env-file .env.full -f docker-compose.full.yml up -d --force-recreate admin && sleep 15 && curl http://localhost:8010/api/v1/health"
```
