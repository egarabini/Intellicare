...
6. **Integração com infraestrutura** existente (Prometheus + Grafana do MODULARIZACAO)

**Contexto**: O IntelliCare já possui infraestrutura de monitoramento (ver `MODULARIZACAO/prometheus.yml`, `grafana-dashboards.yml`, `grafana-datasources.yml`). Este domínio adiciona métricas específicas do módulo de comunicação a essa infraestrutura existente.
...
async def metrics():
    ...
    Scraped pelo Prometheus configurado em MODULARIZACAO/prometheus.yml
    """
    return Response(
...
Adicionar ao `MODULARIZACAO/prometheus.yml`:
...
Adicionar ao `MODULARIZACAO/grafana-datasources.yml`:
...
Adicionar ao `MODULARIZACAO/grafana-dashboards.yml`:
