# intellicare-superz — Especificacao Tecnica
...
```
intellicare-superz/
├── superz/                                 # Modulo principal
│   ├── __init__.py
│   ├── config.py                           # SuperZConfig (dataclass)
│   │
│   ├── api/                                # REST API
│   │   ├── __init__.py
│   │   └── app.py                         # FastAPI app
...
class SearchCache:
    ...
    Prefixos:
    - "superz:tavily:{hash}" → WebSearchResponse (TTL 6h)
    - "superz:pubmed:{hash}" → LiteratureSearchResponse (TTL 24h)
    - "superz:bireme:{hash}" → LiteratureSearchResponse (TTL 24h)
    - "superz:regulatory:{hash}" → list[RegulatoryResult] (TTL 12h)
...
class TavilyRateLimiter:
    ...
    Configuravel via env: SUPERZ_TAVILY_RATE_LIMIT_PER_HOUR
    """
...
CMD ["uvicorn", "superz.api.app:app", "--host", "0.0.0.0", "--port", "8009", "--workers", "2"]
```
...
services:
  intellicare-superz:
    ...
    environment:
      ...
      - SUPERZ_LLM_MODEL=${SUPERZ_LLM_MODEL:-qwen2.5:72b}
      - REDIS_URL=redis://redis:6379/1
      - SUPERZ_PORT=8009
    depends_on:
      redis:
