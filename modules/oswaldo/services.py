...
from modules.oswaldo.contracts import MedicationItem, OswaldoSuggestion, OswaldoSuggestRequest, PrescriptionItem, ReceituarioData
...
async def suggest(ctx: TenantContext, req: OswaldoSuggestRequest) -> OswaldoSuggestion:
    ...
    if is_marie_enabled() and patient_uuid is not None:
        marie_response = await call_marie(
            ...
            inputs={"query": req.chief_complaint, "patient_history": await _get_patient_timeline_summary(ctx, patient_uuid)},
        )
...
def _rule_based_suggestion(req: OswaldoSuggestRequest) -> dict:
    return {"cid10_code": "Z00", "cid10_desc": "Consulta de rotina (preencher manualmente)", "items": [], "model": "rule-based"}
...
FREQ_MAP = {"4/4h": "a cada 4 horas", "6/6h": "a cada 6 horas", "8/8h": "a cada 8 horas", "12/12h": "a cada 12 horas", "1x/dia": "uma vez ao dia", "2x/dia": "duas vezes ao dia", "3x/dia": "três vezes ao dia"}
NUM_EXTENSO = {"1": "um", "2": "dois", "3": "três", "4": "quatro", "5": "cinco", "6": "seis", "7": "sete", "8": "oito"}
ROTA_FORMA = {"comprimido": "via oral", "comp": "via oral", "cápsula": "via oral", "xarope": "via oral", "pomada": "uso tópico", "creme": "uso tópico", "colírio": "uso oftálmico", "injetável": "via intramuscular"}
...
def _expand_numbers(text: str) -> str:
    return re.sub(r"\b([1-8])\b", lambda m: f"{m.group(0)} ({NUM_EXTENSO[m.group(0)]})" if m.group(0) in NUM_EXTENSO else m.group(0), text)
...
def format_posologia(item: dict) -> str:
    text = _expand_numbers(_expand_freq(item.get("posology", "")))
    if not any(text.lower().startswith(v) for v in ("tomar", "aplicar", "usar", "instilar")):
        text = f"Tomar {text}"
    duration = item.get("duration", "")
    if duration:
        text += f" por {_expand_numbers(_expand_freq(duration))}"
...
async def generate_receituario(ctx: TenantContext, prescription_id: int, ptype: str) -> bytes:
    ...
    if pat and pat.get("birth_date"):
        try:
            if isinstance(pat["birth_date"], datetime.date):
                ...
                pat_age = (datetime.date.today() - datetime.datetime.strptime(str(pat["birth_date"])[:10], "%Y-%m-%d").date()).days // 365
        except Exception:
            pass
    meds = [MedicationItem(order=idx, drug_name=item.drug, concentration="", pharmaceutical_form="", quantity=1, quantity_unit="unidade(s)", dosage_instructions=format_posologia(item.model_dump()), route=detect_route(item.posology)) for idx, item in enumerate(rx.items, 1)]
    qr_b64 = generate_qr_base64(f"https://intellicare.ia.br/valida/rx/{prescription_id}?tenant={ctx.tenant_id}")
    ...
    context = {"data": data, "qr_code_b64": qr_b64, "issued_date_br": rx.created_at.strftime("%d/%m/%Y") if rx.created_at else "", "issued_date_extenso": rx.created_at.strftime("%d de %B de %Y") if rx.created_at else ""}
        ...
        if len(medications) > 1:
            ...
            if warnings:
                async with tenant_session(ctx) as db:
                    await db.execute(text("UPDATE prescriptions SET interaction_warnings_count = :count WHERE id = :id"), {"count": len(warnings), "id": rx.id})
    except Exception as exc:
        logger.error("Erro ao verificar interações/salvar KPI durante receituário: %s", exc)
    return pdf_bytes
...
async def _try_sign_pdf(ctx: TenantContext, pdf_bytes: bytes, professional_id: int) -> bytes:
    try:
        ...
        return sign_pdf(pfx_bytes=decrypt_bytes(cert["pfx_encrypted"]), pfx_password=decrypt_text(cert["password_hash"]), pdf_bytes=pdf_bytes)
    except Exception as exc:
        logger.warning("Assinatura digital falhou para professional_id=%s: %s", professional_id, exc)
        return pdf_bytes
