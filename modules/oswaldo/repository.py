...
from modules.oswaldo.contracts import CID10Result, CreatePrescriptionRequest, Prescription, PrescriptionItem
...
async def create_prescription(ctx: TenantContext, req: CreatePrescriptionRequest, author_id: str, author_name: str) -> Prescription:
...
async def get_prescriptions_by_encounter(ctx: TenantContext, encounter_id: str) -> list[Prescription]:
    async with tenant_session(ctx) as db:
        rows = (
            await db.execute(
...
async def get_prescription(ctx: TenantContext, prescription_id: int) -> Prescription | None:
    async with tenant_session(ctx) as db:
        row = (await db.execute(text("SELECT * FROM prescriptions WHERE id = :id"), {"id": prescription_id})).mappings().first()
...
async def get_professional_by_keycloak_id(ctx: TenantContext, keycloak_id: str) -> dict | None:
    async with tenant_session(ctx) as db:
        row = (await db.execute(text("SELECT * FROM professionals WHERE keycloak_id = :id"), {"id": keycloak_id})).mappings().first()
    return dict(row) if row else None


async def get_patient_by_id(ctx: TenantContext, patient_id: str) -> dict | None:
    async with tenant_session(ctx) as db:
        row = (await db.execute(text("SELECT * FROM patients WHERE id = :id"), {"id": int(patient_id)})).mappings().first()
    return dict(row) if row else None


async def get_professional_certificate(ctx: TenantContext, professional_id: int) -> dict | None:
    async with tenant_session(ctx) as db:
        row = (await db.execute(text("SELECT * FROM professional_certificates WHERE professional_id = :pid"), {"pid": professional_id})).mappings().first()
    return dict(row) if row else None
    ...
    async with tenant_session(ctx) as db:
        row = (
            ...
            """
                ...
                {"pid": professional_id, "pfx": pfx_encrypted, "pwd": password_hash, "subj": subject_name, "vu": valid_until},
            )
        ).mappings().first()
    return dict(row)
...
async def delete_professional_certificate(ctx: TenantContext, professional_id: int) -> bool:
    async with tenant_session(ctx) as db:
        result = await db.execute(text("DELETE FROM professional_certificates WHERE professional_id = :pid"), {"pid": professional_id})
