...
3. Tratar incidente de seguranca por credenciais em `./.env.homologacao`.

## Definicao de Pronto (DoD) da V1
1. Nao existem novos arquivos/pipelines com `homologacao` como termo primario.
2. Fluxo GIT -> STAGING roda com scripts oficiais de `./PADRAO_ENTREGA/`.
...
1. Tratar `./.env.homologacao` como credencial exposta.
...
1. Inventario de termos legados:
   - `rg -n "homologacao|HOMOLOGACAO" . docs`
2. Validacao minima antes de push:
   - `pwsh -File ./PADRAO_ENTREGA/GIT/validacao-minima.ps1`
3. Push padronizado:
   - `pwsh -File ./PADRAO_ENTREGA/GIT/push-com-validacao.ps1`
4. Deploy staging com fast-forward only:
   - `pwsh -File ./PADRAO_ENTREGA/STAGING/deploy-module.ps1 -Module <modulo>`
...
6. Rollback padrao:
   - `pwsh -File ./PADRAO_ENTREGA/STAGING/rollback-module.ps1 -Module <modulo>`
