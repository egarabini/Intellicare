# intellicare-minerva — Especificacao Tecnica

**Modulo:** `intellicare-minerva` (MINERVA Forever / MINERVA)
**Versao:** 1.0 (IntelliCare V5)
**Porta:** 8008
**Data:** 2026-02-16
**DEV responsavel:** DEV-MINERVA (independente — nao interfere com outros modulos)
...
| MINERVA Engine | **Surya** | 0.6+ | MINERVA open-source SOTA, roda via Ollama |
...
| Preprocessamento | **Pillow** + **OpenCV** | latest | Melhoria de imagem antes do MINERVA |
...
### Sobre a escolha de MINERVA
- **Surya** foi escolhido sobre DeepSeek-MINERVA por rodar 100% local (Docker), sem API externa, com qualidade SOTA em PT-BR
...
│   │   ├── __init__.py
│   │   ├── ocr_engine.py                  # SuryaMINERVAEngine
...
│   ├── __init__.py
│   ├── conftest.py
│   ├── test_ocr_engine.py                 # Testes do Surya MINERVA
...
### 4.1 SuryaMINERVAEngine

```python
class SuryaMINERVAEngine:
    """
    Engine MINERVA usando Surya (open-source, SOTA).
    ...
    async def extract_text(
        ...
        2. Surya MINERVA — reconhecimento de texto
...
class Llama4VisionEngine:
    ...
    Usado para compreensao de LAYOUT e CONTEXTO, nao apenas MINERVA.
    ...
    Fallback: SuryaMINERVAEngine (sem contexto de layout)
...
class PDFParser:
    ...
    3. SuryaMINERVA — para PDFs escaneados (imagem)
    ...
    async def parse(
        ...
        → Converte paginas em imagem → SuryaMINERVA → pos-processar
...
class OcrMCPServer:
    """
    MCP Server para intellicare-minerva.
    Expoe 6 tools via protocolo MCP sobre HTTP SSE.
    """

    def __init__(self, config: OcrConfig):
        self._server = Server("intellicare-minerva")
        self._config = config
    ...
    def _register_handlers(self):
        ...
        async def handle_list_tools() -> list[Tool]:
            return [
                ...
                Tool(
                    name="ocr_image",
                    description="MINERVA especializado para imagens medicas — prescricoes manuscritas, "
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "enhance_quality": {
                                "type": "boolean",
                                "default": True,
                                "description": "Aplicar melhoria de imagem antes do MINERVA"
                            },
                        },
...
app = FastAPI(title="IntelliCare MINERVA (MINERVA)", version="1.0.0")

@app.get("/api/v1/health")
async def health():
    return {
        "status": "healthy",
        "module": "intellicare-minerva",
        "version": "1.0.0",
        "engines": {
...
class OcrConfig:
    ...
    def from_env(cls) -> "OcrConfig":
        ...
        return cls(
            ...
            vision_model=os.getenv("MINERVA_VISION_MODEL", "llama4:scout"),
            llamaparse_api_key=os.getenv("LLAMAPARSE_API_KEY"),
            chroma_host=os.getenv("CHROMA_HOST", "localhost"),
            lgpd_salt=os.getenv("MINERVA_LGPD_SALT", ""),
        )
```
...
MINERVA_VISION_MODEL=llama4:scout
MINERVA_SURYA_DEVICE=cpu
...
# LGPD
MINERVA_LGPD_SALT=<gerar-com-openssl-rand-hex-32>

# Limites
MINERVA_MAX_FILE_SIZE_MB=50
MINERVA_MAX_PAGES=100
MINERVA_CONFIDENCE_WARN=0.60
...
services:
  intellicare-minerva:
    build: .
    container_name: intellicare-minerva
    ports:
      - "8008:8008"
    environment:
      - OLLAMA_URL=${OLLAMA_URL:-http://host.docker.internal:11434}
      - MINERVA_VISION_MODEL=${MINERVA_VISION_MODEL:-llama4:scout}
      - LLAMAPARSE_API_KEY=${LLAMAPARSE_API_KEY:-}
      - CHROMA_HOST=chromadb
      - CHROMA_PORT=8200
      - MINERVA_LGPD_SALT=${MINERVA_LGPD_SALT}
      - MINERVA_PORT=8008
    volumes:
...
├── laudo_lab_escaneado.pdf    # PDF escaneado (imagem) — testa MINERVA completo
├── sumario_alta.pdf           # Sumario de alta com CIDs e medicamentos
├── prescricao_manuscrita.jpg  # Prescricao para testar MINERVA de manuscrito
...
2. MINERVA Engine (Surya + tests)                                 [Dia 2]
