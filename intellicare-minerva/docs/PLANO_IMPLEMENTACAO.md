# intellicare-minerva — Plano de Implementacao
...
Entregar o modulo `intellicare-minerva` (MINERVA) como MCP Server + API REST para MINERVA, extracao estruturada de documentos medicos e busca semantica.
...
3. Pipeline tecnico:
- MINERVA engine (Surya + fallback)
- parser PDF (PyMuPDF + fallback MINERVA)
...
## Fase 2 — MINERVA e Parser (Dias 2-3)

1. `SuryaMINERVAEngine` com fallback local
2. `PDFParser` com deteccao digital x escaneado
3. preprocessamento de imagem
4. testes unitarios de MINERVA/PDF
...
1. Dependencia de modelos MINERVA/LLM
