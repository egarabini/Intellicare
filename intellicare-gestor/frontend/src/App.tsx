...
function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        // In development, skip Keycloak and use mock user
        if (import.meta.env.DEV && import.meta.env.VITE_SKIP_AUTH === 'true') {
          setUser({
            id: 'dev-user',
            name: 'Desenvolvedor',
            email: 'dev@intellicare.ia.br',
            roles: ['TENANT_GESTOR'],
            tenantId: 'dev-tenant',
            tenantName: 'Secretaria Municipal de Saúde',
          });
        ...
        console.error('[App] Auth initialization failed');
        setAuthError(true);
      } finally {
        setLoading(false);
      }
