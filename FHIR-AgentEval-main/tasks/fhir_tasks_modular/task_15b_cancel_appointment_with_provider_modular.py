# task_15b_cancel_appointment_with_provider_modular.py
...
class CancelAppointmentWithProviderTaskModular(TaskInterfaceModular):
    def get_task_id(self) -> str:
        return "15b"

    def get_task_name(self) -> str:
        return "Cancel Next Appointment with Provider"
    ...
    def get_prompt(self) -> str:
        ...
        return f"""
Cancel Patient {' '.join(patient_given)} {patient_family}'s next coming appointment with Dr. {' '.join(practitioner_given)} {practitioner_family}.
...
"""
    ...
    def prepare_test_data(self) -> None:
        try:
            ...
            # Create a second practitioner and appointment
            practitioner2 = {
                "resourceType": "Practitioner",
                "id": "PROVIDER002",
    ...
    def execute_human_agent(self) -> ExecutionResult:
        # Find the next appointment for the specified patient with the specified practitioner
        params = {
    ...
    def validate_response(self, execution_result: ExecutionResult) -> TaskResult:
        try:
            # Verify that the appointment with the specified practitioner is cancelled
            params = {
                "patient": "Patient/PAT001",
                "practitioner": "Practitioner/PROVIDER001",
                "status": "cancelled",
            ...
            # Verify that the other appointment is still booked
            params = {
                "patient": "Patient/PAT001",
    ...
    def validate_response_light(self, execution_result: ExecutionResult) -> TaskResult:
        try:
            ...
            # Additional assertions - check tags exist
            response_msg = execution_result.response_msg.strip()

            appointment_id = response_msg.split("<APPOINTMENT>")[1].split("</APPOINTMENT>")[0]
            slot_id = response_msg.split("<SLOT_ID>")[1].split("</SLOT_ID>")[0]

            # Verify the appointment exists and has status (not checking specific value)
            appt_resp = requests.get(f"{self.FHIR_SERVER_URL}/Appointment/{appointment_id}", headers=self.HEADERS)
            ...
            # Verify the slot exists and has status (not checking specific value)
            slot_resp = requests.get(f"{self.FHIR_SERVER_URL}/Slot/{slot_id}", headers=self.HEADERS)
