# task_11b_search_most_recent_slots_by_language_modular.py
...
class FindAvailableSlotsTaskModular(TaskInterfaceModular):
    ...
    def get_prompt(self) -> str:
        required_language = self.get_param("required_language")
        required_gender = self.get_param("required_gender")
        required_city = self.get_param("required_city")
        return f"""
Find the most recent available slot from any providers who can speak {required_language}, is a {required_gender} and is working at {required_city}. 
Return the Slot ID from the FHIR server. 
Using the following format: <slot_id>SAMPLE_SLOT_ID</slot_id>
"""

    def get_param_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "required": ["required_language", "required_gender", "required_city"],
            "properties": {
                "required_language": {
                    "type": "string",
                    "description": "Required language code for practitioners",
                    "examples": ["es", "en", "zh", "hi"],
                    "default": "es",
                },
                "required_gender": {
                    "type": "string",
                    "description": "Required gender for practitioners",
                    "examples": ["female", "male"],
                    "default": "female",
                },
                "required_city": {
                    "type": "string",
                    "description": "Required city for practitioners",
                    "examples": ["Boston", "Los Angeles", "Chicago"],
                    "default": "Boston",
                },
                "practitioner_count": {
                    "type": "integer",
                ...
                "practitioner_languages": {
                    ...
                    "items": {"type": "array", "items": {"type": "string"}},
                    "examples": [[["en"], ["en", "zh", "es"], ["en", "es"], ["en", "hi"]]],
                    "default": [["en"], ["en", "zh", "es"], ["en", "es"], ["en", "hi"]],
                },
    ...
    def prepare_test_data(self) -> None:
        try:
            ...
            practitioner_languages = self.get_param("practitioner_languages", [["en"], ["en", "zh", "es"], ["en", "es"], ["en", "hi"]])
    ...
    def execute_human_agent(self) -> ExecutionResult:
        ...
        # Search for practitioners who meet the criteria
        params = {
            "gender": required_gender,
            "address-city": required_city,
            "communication": required_language,
        }
        response = requests.get(
            f"{self.FHIR_SERVER_URL}/Practitioner", headers=self.HEADERS, params=params
        )
        ...
        if response.status_code != 200:
            ...
            if not response_json.get("entry"):
                return ExecutionResult(
                    execution_success=False, response_msg="No practitioners found"
                )
            else:
                ...
                # Search for most recent slot among all available slots for each practitioner
                ...
                for practitioner_id in practitioner_ids:
                    ...
                    if response.status_code != 200:
                        ...
                        if not response_json.get("entry"):
                            ...
                            tagged_response = f"<slot_id>{most_recent_slot['resource']['id']}</slot_id>"                                                
                            return ExecutionResult(
