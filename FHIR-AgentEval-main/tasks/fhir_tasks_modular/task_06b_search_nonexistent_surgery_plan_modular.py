# task_06b_search_nonexistent_surgery_plan_modular.py
...
class SearchNonexistentSurgeryPlanTaskModular(TaskInterfaceModular):
    ...
    def get_param_schema(self) -> Dict[str, Any]:
        return {
            ...
            "properties": {
                "patient_id": {
                    "type": "string",
                    "description": "Patient ID to search surgery plans for (should have no plans in search window)",
                    "examples": ["PAT002"],
                    "default": "PAT002",
                },
                "procedure_code": {
                ...
                "surgery_plan_id": {
                    ...
                    "examples": ["APPENDECTOMY-REQUEST-002"],
                    "default": "APPENDECTOMY-REQUEST-002",
                },
                "days_ahead": {
                    "type": "integer",
                    "description": "Number of days ahead to schedule the surgery (outside search window)",
                    "examples": [21],
                    "default": 21,
                    "minimum": 1,
                },
...
Search and find if patient id={patient_id} has any surgery plan {search_days_ahead} days from now.

If found, return the first surgery plan's ID using the following format: <SURGERY_PLAN>plan_id</SURGERY_PLAN>.
...
"""
    ...
    def prepare_test_data(self) -> None:
        try:
            ...
            days_ahead = self.get_param("days_ahead")
            ...
            birth_date = self.get_param("patient_birth_date", "1991-06-15")
            ...
            # Create service request using template parameters (outside search window)
            procedure_code = self.get_param("procedure_code", "80146002")
            procedure_display = self.get_param("procedure_display", "Appendectomy")
            surgery_plan_id = self.get_param("surgery_plan_id", "APPENDECTOMY-REQUEST-002")
    ...
    def execute_human_agent(self) -> ExecutionResult:
        ...
        if response_json.get("total", 0) > 0:
            return ExecutionResult(
                ...
                response_msg=(
                    f"Found {response_json['total']} surgery plan(s) for patient {patient_id}: "
                    f"<SURGERY_PLAN>{response_json['entry'][0]['resource']['id']}</SURGERY_PLAN>"
                ),
            )
        else:
            return ExecutionResult(
                execution_success=True, response_msg="No surgery plan found"
            )
    ...
    def validate_response(self, execution_result: ExecutionResult) -> TaskResult:
        try:
            ...
            # Tighten validation: check tool calls are correct
