# task_21a_add_supporting_info_modular.py
...
class AddSupportingInfoTaskModular(TaskInterfaceModular):
    def get_task_id(self) -> str:
        return "21a"

    def get_task_name(self) -> str:
        return "Add supporting information to an existing service request"
    ...
    def get_prompt(self) -> str:
        ...
        return f"""
Add any relevant diagnostic reports for patient {patient_given} {patient_family} as supporting information for their upcoming test order.
...
"""
    ...
    def prepare_test_data(self) -> None:
        try:
            ...
            # Create first patient - John Doe (target patient)
            patient1 = {
            ...
            # Create second patient - Jane Smith (distractor)
            patient2 = {
                "resourceType": "Patient",
                "id": "PAT-003",
                "name": [{"use": "official", "family": "Smith", "given": ["Jane"]}],
                "birthDate": "1985-05-15",
                "telecom": [{"system": "phone", "value": "987-654-3210"}],
                "address": [{"line": ["456 Oak Ave"], "city": "Boston", "state": "MA"}]
            }
            ...
            # Create service request for John Doe
            service_request = {
                ...
                "requester": {"reference": "Practitioner/PROVIDER-001"}
            }
            self.upsert_to_fhir(service_request)

            # Create diagnostic report for John Doe
            diagnostic_report1 = {
                ...
                "subject": {"reference": "Patient/PAT-002"},
                "conclusion": "No pathogenic variants identified."
            }
            self.upsert_to_fhir(diagnostic_report1)

            # Create diagnostic report for Jane Smith (distractor)
            diagnostic_report2 = {
                "resourceType": "DiagnosticReport",
                "id": "DR-002",
                "status": "final",
                ...
                "conclusion": "Pathogenic variant detected in BRCA1 gene."
            }
            self.upsert_to_fhir(diagnostic_report2)
    ...
    def execute_human_agent(self) -> ExecutionResult:
        ...
        # 2) Retrieve the diagnostic report for this patient
        ...
        if len(dr_resp.json().get('entry', [])) == 0:
            return ExecutionResult(
                execution_success=False,
                response_msg="No diagnostic report found for this patient"
            )

        # Build supporting info references
        entries = dr_resp.json().get("entry", [])
        supporting_info_ref = [
            {"reference": f"DiagnosticReport/{entry['resource']['id']}"}
        ...
        # 4) Update service request with supporting info
        service_request['supportingInfo'] = supporting_info_ref
        ...
        return ExecutionResult(
            ...
            response_msg=f"Successfully added supporting information to ServiceRequest <RESOURCE>{service_request['id']}</RESOURCE>"
        )
    ...
    def validate_response(self, execution_result: ExecutionResult) -> TaskResult:
        try:
            ...
            # Validate exactly one diagnostic report is added
            ...
            # Validate it's the correct diagnostic report for John Doe (DR-001), not Jane Smith's (DR-002)
            assert refs[0] == "DiagnosticReport/DR-001", (
                f"Expected DiagnosticReport/DR-001 (John Doe's report), but got {refs[0]}"
            )
