...
Documento consolidado para treinamento e operacao dos perfis do IntelliCare, com base no que ja foi desenvolvido ate a `DEM-063`.
...
### 4.2 Fluxo principal: criar novo tenant

Campos:

- `Nome` (obrigatorio)
- `Slug` (gerado automaticamente; imutavel apos criacao)
...
- Tenant criado com sucesso e disponivel na lista.

### 4.3 Prints desta secao
...
2. Abrir encontro se necessario.
3. Registrar conteudo em `Notas Florence`.
4. Registrar CID-10 e prescricao em `Prescrição`.
5. Revisar e `Fechar Encontro`.
6. Gerar `PDF Clínico`.
...
- Status staging clinico: `DEM-064 em andamento`
- Local do arquivo: `DOCUMENTACAO/MANUAL_USUARIO_INTELLICARE_COMPLETO.md`
