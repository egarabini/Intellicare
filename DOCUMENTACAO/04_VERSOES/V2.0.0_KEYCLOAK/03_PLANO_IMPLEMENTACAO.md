...
PostgreSQL +    roles +         configure_auth  authService     smart/ lib ✅   HTTPS +
Keycloak up     clients +       em todos        PKCE fix        Montar router   monitoring
                mappers +       os módulos      TenantCtx       no GRAHAME
                realm.json      + Wanda M2M
                                exchange

✅ Concluído   ✅ Concluído   ✅ Concluído   🔄 Em Progresso  🔄 Em Progresso  ⬜ Pendente
```
...
- ✅ `rememberMe: false` — desabilitado para evitar pré-preenchimento da tela de login
  > **Fix 2026-03-07:** realm.json editado e reimportado via `kc.sh import --file ...` no container de produção (`167.86.97.142`)
...
### Reimportar Realm em Produção (após editar realm.json)

Quando o realm.json for alterado após o primeiro deploy, o `--import-realm` do startup ignora
o arquivo (realm já existe). Para forçar a reimportação use o CLI direto no container:

```bash
# Copiar realm.json atualizado para o servidor
scp intellicare-auth/keycloak/import/bemcuidar-realm.json \
    root@167.86.97.142:/tmp/bemcuidar-realm.json

# Importar no container de produção (sobrescreve realm existente)
docker exec keycloak-intellicare \
    /opt/keycloak/bin/kc.sh import \
    --file /tmp/bemcuidar-realm.json \
    --override true

# Verificar que a alteração foi aplicada (ex: rememberMe)
curl -s https://auth.intellicare.ia.br/realms/bemcuidar \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print('rememberMe:', d.get('rememberMe','not_found'))"
```

> ⚠️ O `--override true` sobrescreve o realm inteiro. Sempre fazer backup antes:
> `docker exec keycloak-intellicare /opt/keycloak/bin/kc.sh export --realm bemcuidar --file /tmp/backup-$(date +%Y%m%d).json`
...
## 📋 Fase 4: SMART-on-FHIR 2.0 (🔄 Em Progresso)

### Dependência

Fase 3 completa + módulo `intellicare-grahame` funcional.

### O que já existe

- ✅ `intellicare_auth/smart/` — **Módulo completo** com `EHRLaunchHandler`, `StandaloneLaunchHandler`, `smart_router`, `SmartConfiguration`, `scope_translator`
- ✅ `smart_router` expõe `/.well-known/smart-configuration`, `/smart/launch`, `/smart/launch/validate/{token}`

### O que falta

- [ ] Montar `smart_router` no `app.py` do GRAHAME
- [ ] Configurar `KEYCLOAK_BASE_URL` e `FHIR_BASE_URL` no `.env` do GRAHAME
- [ ] Validar patient context em FHIR queries (usar `LaunchContext.patient_id`)
- [ ] Testar com client SMART externo (Inferno Test Suite)

### Tarefas Restantes

```
T4.1 - [✅ FEITO] smart/ implementado em intellicare-auth
T4.2 - Montar smart_router no GRAHAME app.py
T4.3 - Setar KEYCLOAK_BASE_URL (NÃO KEYCLOAK_SERVER_URL) no .env do GRAHAME
T4.4 - Validar patient context em FHIR queries
T4.5 - Teste com Inferno Test Suite
```
...
| T4.1 | 4 | smart/ módulo em intellicare-auth | ✅ | Backend |
| T4.2 | 4 | Montar smart_router no GRAHAME app.py | ⬜ | Backend |
| T4.3 | 4 | KEYCLOAK_BASE_URL no .env do GRAHAME | ⬜ | Backend |
| T4.4 | 4 | Validar patient context + Standalone launch | ⬜ | Backend |
...
|------------|-------|---------|---------|
| ✅ RESOLVIDO | `authService.ts` usava realm `'intellicare'` no fallback | Login falhava sem VITE_KEYCLOAK_REALM | Corrigido para `\|\| 'bemcuidar'` |
| ✅ RESOLVIDO | `validate_audience` causaria falha nos módulos backend | JWT `aud[]` não incluía client_id de cada módulo | `config.py` alterado: `validate_audience: bool = Field(default=False)` |
| ✅ RESOLVIDO | `TenantContext.tsx` não tinha UI de seleção de tenant | Modal de seleção ausente — usuário ficava preso | `TenantContext.tsx` reescrito com modal de seleção de polo + estado `showSelector` / `tenants` expostos no contexto |
| ✅ RESOLVIDO | `docker-compose.keycloak.yml` usava `start-dev` | Não apto para produção | Alterado para `start` + rate limiting Traefik inline (`kc-ratelimit: avg=5, burst=10`) |
| 🟡 MÉDIO | `smart/router.py` usa `KEYCLOAK_BASE_URL` (não `KEYCLOAK_SERVER_URL`) | `.env` do GRAHAME precisa da var correta | Setar `KEYCLOAK_BASE_URL` no `.env` do GRAHAME |
| 🟡 MÉDIO | `pyproject.toml` de `intellicare-auth` tem `packages = [{include = "auth", from = "src"}]` | Path incorreto — módulo real é `intellicare_auth/` na raiz | Corrigir para `packages = [{include = "intellicare_auth"}]` |
| 🟡 MÉDIO | `TenantContext.fetchTenantProfile` ainda usa mock de branding | Cores/módulos hardcoded — não reflete dados reais do gestor | Implementar chamada real à API do gestor após Fase 3 completa |
