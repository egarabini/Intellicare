...
> **Atualizado em 2026-03-08 (DEM-003):** admin e gestor agora servem React SPAs
> via nginx, não mais o dashboard.html Python. Ver seção de decisões arquiteturais.
...
admin.intellicare.ia.br
  nginx serve React SPA (porta 3003 Docker) — intellicare-admin-frontend
  nginx proxia /api/* → FastAPI intellicare-admin (porta 8010)
  Auth: Keycloak PKCE, cliente intellicare-admin, realm intellicare
  Acesso: role PLATFORM_ADMIN
  Finalidade: gestão da plataforma (tenants, gestores, planos, billing)
  Painéis de doenças (establishments registrados):
    DRC | Diabetes | Hipertensão | Câncer
  Páginas exclusivas vs. gestor: Tenants (cross-tenant) + Consolidado Nacional
  Stack: React 19 + Vite 7 + TypeScript + Tailwind 4 + Recharts + Leaflet

gestor.intellicare.ia.br
  nginx serve React SPA (porta 3002 Docker) — intellicare-gestor-frontend
  nginx proxia /api/* → FastAPI intellicare-gestor (porta 8011)
  Auth: Keycloak PKCE, cliente intellicare-gestor, realm intellicare
...
| 2026-03-08 | Admin e gestor servem React SPAs via nginx (não dashboard.html Python) | nginx + SPA é o padrão correto para produção; FastAPI serve apenas /api/* |
| 2026-03-08 | Endpoints de disease dashboard ficam no Donabedian | Donabedian é o módulo de qualidade e indicadores — dono natural desses dados |
